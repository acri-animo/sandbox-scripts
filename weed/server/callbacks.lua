		...
		if pid then
			if checkNearPlant(source, pid) then
				if _plants[pid] ~= nil then
					local char = Fetch:CharacterSource(source)
					if char ~= nil then
						local strain = _plants[pid].plant.strain
						local stage = Plants[strain][getStageByPct(_plants[pid].plant.growth)]
						
						if stage.harvestable then
							if _plants[pid].plant.isMale then
								local luck = math.random(100)
								local giving = "weedseed_" .. strain .. "_male"
								if luck >= (100 - Config.FemSeedChance) then
									giving = "weedseed_" .. strain .. "_female"
								end
								if
									Inventory:AddItem(
										char:GetData("SID"),
								...
								if Inventory:AddItem(char:GetData("SID"), strain .. "_bud", t, {}, 1) then
									Weed.Planting:Delete(pid)
									cb(true)
								else
									cb(false)
