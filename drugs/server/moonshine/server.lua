...
_DRUGS.Moonshine = {
    ...
    Barrel = {
        ...
        CreatePlaced = function(self, owner, coords, heading, created, brewData)
            ...
            local ready = os.time() + (1 * 1 * 24 * 1) -- BARREL BREW TIMER

            local barrelId = MySQL.insert.await("INSERT INTO placed_moonshine_barrels (owner, placed, ready, expires, coords, heading, brew_data, pickup_ready) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", {
                owner,
                os.time(),
                ready,
                created + itemInfo.durability,
...
AddEventHandler("Drugs:Server:Startup", function()
    ...
    Vendor:Create("MoonshineSeller", "ped", "Karen", `S_F_Y_Bartender_01`, {
        coords = vector3(-1101.340, 4877.036, 215.462),
        heading = 154.156,
    ...
    Callbacks:RegisterServerCallback("Drugs:Moonshine:GetBarrelDetails", function(source, data, cb)
        ...
        if char ~= nil then
            ...
            if data and barrel ~= nil then
                ...
                if barrel.ready ~= nil and os.time() < barrel.ready then
                    local timeUntil = barrel.ready - os.time()
                    table.insert(menu.main.items, {
                        label = "Aging Process Still In Progress",
                        description = string.format("Finishes At %s (in about %s)", os.date("%m/%d/%Y %I:%M %p", barrel.ready), GetFormattedTimeFromSeconds(timeUntil)),
                    })
                else
                    table.insert(menu.main.items, {
                        label = "Aging Process Finished",
                        description = string.format("Finished At %s", os.date("%m/%d/%Y %I:%M %p", barrel.ready)),
                    })
                end
    ...
    Callbacks:RegisterServerCallback("Drugs:Moonshine:PickupBrew", function(source, data, cb)
        ...
        if char ~= nil then
            ...
            if data and _placedBarrels[data] ~= nil and _placedBarrels[data].pickupReady then
                ...
                Execute:Client(source, "Notification", "Error", "The moonshine is not ready for pickup yet.")
                cb(false)
            end
        else
            cb(false)
