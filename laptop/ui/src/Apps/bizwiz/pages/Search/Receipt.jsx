...
export default ({ onNav }) => {
		...
		() =>
			throttle(async (value) => {
				...
				try {
					...
					if (res) {
						const filteredResults = res?.sort(
							(a, b) => b.time - a.time,
						);
	...
	const confirmDeleteAll = async (e) => {
		...
		try {
			let res = await (await Nui.send('BusinessReceiptDeleteAll')).json();
		...
		<div className={classes.wrapper}>
					...
					<Grid container spacing={1}>
						<Grid item xs={canClearReceipts ? 8 : 10}>
							<TextField
								...
								error={err}
								helperText={
									err ? 'Error Performing Search' : null
								}
								label="Search"
								InputProps={{
									endAdornment: (
			...
			>
				<p>
					...
					receipts? This is not reversible!
				</p>
			</Modal>
		</div>
	);
