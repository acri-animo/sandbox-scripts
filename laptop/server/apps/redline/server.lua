...
AddEventHandler("Laptop:Server:RegisterMiddleware", function()
			...
			if item.MetaData.Event and _races[item.MetaData.Event] then
				...
				if alias then
					for k, v in ipairs(_races) do
						if v.state == 0 then
							if _races[k].racers[alias] ~= nil then
								...
								TriggerClientEvent("Laptop:Client:Redline:LeaveRace", -1, k, alias)
							end
						end
					end
					...
					then
						...
						TriggerClientEvent("Laptop:Client:Redline:JoinRace", -1, item.MetaData.Event, alias, _races[item.MetaData.Event].racers[alias])
	...
	Vendor:Create("RaceGear", "poly", "Race Gear", false, {
		coords = vector3(-1156.7, -1516.79, 10.63),
		length = 0.6,
		width = 0.6,
		options = {
			heading = 35,
	...
	Middleware:Add("Characters:Spawning", function(source)
		local char = Fetch:CharacterSource(source)
		TriggerLatentClientEvent("Laptop:Client:Redline:StoreTracks", source, 50000, _tracks)
		TriggerClientEvent("Laptop:Client:Redline:Spawn", source, {
			races = _races,
		})
	end, 2)
...
AddEventHandler("Laptop:Server:UpdateProfile", function(source, data)
...
function ReloadRaceTracks()
	LoadTracks()
	TriggerLatentClientEvent("Laptop:Client:Redline:StoreTracks", -1, 50000, _tracks)
end
			...
			for k, v in pairs(_races) do
				if v.state == 0 then
					if v.host_id == char:GetData("SID") then
						...
						TriggerClientEvent("Laptop:Client:Redline:CancelRace", -1, k)
					else
...
function FinishRace(id, forceEnd)
	...
	if (_races[key].completed ~= nil and #_races[key].completed > 0) or forceEnd then
		...
		for placing, alias in ipairs(_races[key].completed or {}) do
			...
			table.insert(queries, {
				query = "UPDATE redline_racer_history SET total_races = total_races + 1 WHERE sid = ?",
				values = {
					_races[key].racers[alias].sid,
				},
			})
		...
		if updateFastest then
			...
			TriggerClientEvent("Laptop:Client:Redline:StoreSingleTrack", -1, racedTrack, _tracks[racedTrack])
		end
	...
	for k, v in pairs(_races[key].racers) do
		if v.source ~= nil then
			TriggerClientEvent("Laptop:Redline:NotifyDNF", v.source, id)
		end
	end

	TriggerClientEvent("Laptop:Client:Redline:FinishRace", -1, key, _races[key])
...
RegisterServerEvent("Laptop:Redline:FinishRace", function(nId, data, laps, plate, vehName)
	local src = source
...
AddEventHandler("Laptop:Server:RegisterCallbacks", function()
	Callbacks:RegisterServerCallback("Laptop:Redline:GetTrack", function(src, data, cb)
		for k, v in ipairs(_tracks) do
			if v.id == data then
				cb(v)
				return
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:SaveTrack", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		local alias = char:GetData("Profiles").redline.name
		...
		if alias ~= nil then
			...
			TriggerClientEvent("Laptop:Client:Redline:StoreSingleTrack", -1, doc.id, {
				id = doc.id,
				Name = data.Name,
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:DeleteTrack", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		local alias = char:GetData("Profiles").redline.name
		...
		if alias ~= nil and isAdmin then
			...
			TriggerClientEvent("Laptop:Client:Redline:StoreSingleTrack", -1, tonumber(data), nil)
			cb(true)
		else
			cb(false)
		end
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:ResetTrackHistory", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		local alias = char:GetData("Profiles").redline.name
		if alias ~= nil then
			...
			TriggerClientEvent("Laptop:Client:Redline:StoreSingleTrack", -1, data, _tracks[data])
		else
			cb(false)
		end
	end)

	Callbacks:RegisterServerCallback("Laptop:Redline:CreateRace", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		if hasValue(char:GetData("States") or {}, "RACE_DONGLE") then
			if char:GetData("Profiles")?.redline then
				...
				if tmp ~= nil then
					...
					for k, v in pairs(Fetch:AllCharacters()) do
						if v ~= nil then
							TriggerClientEvent("Laptop:Client:Redline:CreateRace", v:GetData("Source"), data)
							local dutyData = Jobs.Duty:Get(v:GetData("Source"))
							if
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:CancelRace", function(src, key, cb)
		local char = Fetch:CharacterSource(src)
		if
		...
		then
			...
			TriggerClientEvent("Laptop:Client:Redline:CancelRace", -1, key)
			cb(true)
		else
			cb(false)
		end
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:StartRace", function(src, key, cb)
		local char = Fetch:CharacterSource(src)
		if
			_races[key].host_id == char:GetData("SID")
			and hasValue(char:GetData("States") or {}, "RACE_DONGLE")
		then
			...
			then
				...
				if dist > 25 then
					...
					cb(true)
					TriggerClientEvent("Laptop:Client:Redline:StartRace", -1, key)
				end
			end
		else
			cb(false)
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:EndRace", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		local key = tostring(data)
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:JoinRace", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		local alias = char:GetData("Profiles")?.redline?.name
		...
		if alias ~= nil and _races[key].state == 0 then
			...
			then
				...
				local profilePicture = MySQL.query.await(
					'SELECT `picture` FROM `character_app_profiles` WHERE `sid` = ? AND `app` = ?',
					{ char:GetData("SID"), 'redline' }
				)

				_races[key].racers[alias] = {
					source = src,
					sid = char:GetData("SID"),
					picture = profilePicture and #profilePicture > 0 and profilePicture[1].picture or nil, -- Include the picture, or nil if not found
				}
				
				cb(_races[key])
				TriggerClientEvent("Laptop:Client:Redline:JoinRace", -1, key, alias, _races[key].racers[alias])
			end
		else
			cb(false)
		end
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:LeaveRace", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		local alias = char:GetData("Profiles").redline.name
		local key = tostring(data)
		if alias ~= nil and _races[key].state == 0 then
			_races[key].racers[alias] = nil
			TriggerClientEvent("Laptop:Client:Redline:LeaveRace", -1, key, alias)
			cb(true)
		else
			cb(false)
		end
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:RemoveRacer", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		if char ~= nil then
			local sid = char:GetData("SID")
			local alias = char:GetData("Profiles")?.redline?.name
			...
			if alias ~= nil and _races[key].state == 0 and _races[key].host_id == sid then
				if data.alias ~= alias then
					...
					if tSid ~= nil then
						...
						if tChar ~= nil then
							TriggerClientEvent("Laptop:Client:Redline:RemovedFromRace", tChar:GetData("Source"))
						end

						cb(true)
						TriggerClientEvent("Laptop:Client:Redline:LeaveRace", -1, key, data.alias)
					else
						cb(false)
					end
				else
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:SendInvite", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		if char ~= nil then
			local sid = char:GetData("SID")
			local alias = char:GetData("Profiles")?.redline?.name
				...
				if data.alias ~= alias and not _raceInvites[key][string.lower(data.alias)] then
					...
					if tSid ~= nil then
						...
						if tChar ~= nil then
							...
							TriggerClientEvent("Laptop:Client:Redline:ReceiveInvite", tChar:GetData("Source"), _raceInvites[key][string.lower(data.alias)])
							cb(true)
						else
							cb(false)
						end
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:AcceptInvite", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		if char ~= nil then
			local sid = char:GetData("SID")
			local alias = char:GetData("Profiles")?.redline?.name
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:DeclineInvite", function(src, data, cb)
		local char = Fetch:CharacterSource(src)
		if char ~= nil then
			local sid = char:GetData("SID")
			local alias = char:GetData("Profiles")?.redline?.name
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:GetProfileData", function(src, data, cb)
		local sid = data.sid
		if not sid then
			cb(false)
			return
		...
		local query = [[
			SELECT 
				SUM(CASE WHEN placing = 1 THEN 1 ELSE 0 END) AS totalFirst,
				SUM(CASE WHEN placing = 2 THEN 1 ELSE 0 END) AS totalSecond,
				SUM(CASE WHEN placing = 3 THEN 1 ELSE 0 END) AS totalThird,
				COUNT(*) AS totalRaces,  -- Count the rows to get the total number of races
				(
					SELECT vehicle
					FROM redline_racer_history
					WHERE sid = ?
					GROUP BY vehicle
					ORDER BY COUNT(*) DESC
					LIMIT 1
		...
		local success, racerData = pcall(function()
			return MySQL.query.await(query, { sid, sid })
		end)

		local profilePicture = MySQL.query.await(
			'SELECT `picture`, `bio` FROM `character_app_profiles` WHERE `sid` = ? AND `app` = ?',
			{ sid, 'redline' }
		)

		if profilePicture and #profilePicture > 0 then
			racerData[1].picture = profilePicture[1].picture
			racerData[1].bio = profilePicture[1].bio
		end

		if success and racerData and #racerData > 0 then
			cb(racerData[1])
		else
			cb({ 
				totalFirst = 0, 
	...
	Callbacks:RegisterServerCallback("Laptop:Redline:UpdateProfileData", function(src, data, cb)
		local picture = data.image
		local bio = data.bio
		local sid = data.sid

		local affectedRows = MySQL.update.await('UPDATE character_app_profiles SET picture = ?, bio = ? WHERE sid = ? AND app = ?', {
			picture,
			bio,
			sid,
			'redline'
	...
	Inventory.Items:RegisterUse("racing_invite", "LSRACE", function(source, item, itemData)
		local char = Fetch:CharacterSource(source)
		...
		if char ~= nil then
			if not pState.onDuty or not _blacklistedJobs[pState.onDuty] then
				...
				local apps = char:GetData("LaptopApps")

				if apps then
					local installedRedline = hasValue(apps.installed, "redline")
					local homeApps = hasValue(apps.home, "redline")

					if not installedRedline and not homeApps then
						if Inventory.Items:RemoveSlot(item.Owner, item.Name, 1, item.Slot, 1) then
							table.insert(apps.installed, "redline")
							table.insert(apps.home, "redline")
							char:SetData("LaptopApps", apps)

							Phone.Email:Send(
								source,
								"Redline Racing",
								os.time(),
								"Welcome to Redline Racing!",
								string.format(
									[[
									Hello %s<br /><br />
									Welcome to Redline Racing!<br /><br /> 
									You can now access the Redline Racing app via laptop!<br />
									- Anonymous
								]],
									char:GetData("First")
								)
							...
							Citizen.SetTimeout(5000, function()
								Laptop.Notification:Add(
									source,
									"Redline Racing",
									"You have installed the Redline Racing app.",
									os.time() * 1000,
									6000,
									"redline",
									{
					...
					print("LaptopApps not found in character data.")
					return
				end
			end
		end
