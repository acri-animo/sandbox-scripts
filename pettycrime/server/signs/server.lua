...
AddEventHandler("PettyCrime:Server:Setup", function()
	...
	Middleware:Add("Characters:Spawning", function(source)
		TriggerLatentClientEvent("PettyCrime:Signs:GetObjects", source, 50000, objects)
	end)

	Callbacks:RegisterServerCallback("PettyCrime:Signs:RemoveSign", function(source, data, cb)
		...
		if char and not atSign then
			...
			if #(sourceCoords - data.coords) < 4 then
				...
				TriggerClientEvent("PettyCrime:Client:DeleteSign", -1, data)
...
RegisterNetEvent("PettyCrime:Server:Signs:AlertPolice", function(coords)
	local src = source
