...
RegisterNetEvent("PettyCrime:Signs:GetObjects", function(incObjects)
	objects = incObjects
end)
		...
		Targeting:AddObject(v.model, "user-secret", {
			{
				...
				icon = "eye-evil",
				event = "PettyCrime:Client:Signs:StealSign",
				data = {
					label = v.label,
					model = v.model,
					item = v.item,
...
AddEventHandler("PettyCrime:Client:Signs:StealSign", function(data, entity)
	...
	if math.random(100) >= 85 then
		...
		if coords ~= nil then
			Citizen.SetTimeout(1000, function()
				TriggerServerEvent("PettyCrime:Server:Signs:AlertPolice", coords)
			end)
		end
	end
...
RegisterNetEvent("PettyCrime:Client:DeleteSign", function(object)
